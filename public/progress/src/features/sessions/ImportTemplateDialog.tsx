...
export default function ImportTemplateDialog({ open, onClose, session, weekNumber, onImported }: { open: boolean; onClose: ()=>void; session: Session; weekNumber: number; onImported: (updated: Session, count: number, name: string) => void }){
  ...
  if (!open) return null
  return (
    <div className="fixed inset-0 bg-black/60 grid place-items-center" onClick={onClose}>
      <div className="bg-card rounded-2xl p-4 shadow-soft w-full max-w-sm" onClick={e=>e.stopPropagation()}>
        ...
        <label className="space-y-1 block mb-2">
          ...
          <select className="w-full bg-slate-800 rounded-xl px-3 py-2" value={templateId} onChange={e=>setTemplateId(e.target.value)}>
        ...
        {!append && confirming && (
          <div className="bg-amber-900/30 border border-amber-700 text-amber-300 text-sm rounded p-2 mb-2">This will replace all exercises for this session.</div>
        )}
        <div className="flex justify-end gap-2">
          <button className="bg-slate-700 px-3 py-2 rounded-xl" onClick={onClose}>Cancel</button>
          <button className="bg-brand-600 hover:bg-brand-700 px-3 py-2 rounded-xl disabled:opacity-50" disabled={!templateId} onClick={doImport}>Import</button>
        </div>
      </div>
    </div>
  )
