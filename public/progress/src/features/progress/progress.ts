...
export function getWeekCompletion(
...
): { completedDays: number, totalDays: number, percent: number, dayMap: Record<DayId, boolean> } {
  const target = Math.max(3, Math.min(6, opts?.weeklyTargetDays ?? 6))
  const dayMap: Record<DayId, boolean> = { 0:false,1:false,2:false,3:false,4:false,5:false,6:false }
  const inWeek = sessions.filter(s => (s.phaseNumber ?? s.phase ?? 1) === phaseNumber && s.weekNumber === (weekNumber as any))
  ...
  const completedDays = [0,1,2,3,4,5].reduce((a,d)=> a + (dayMap[d as DayId] ? 1 : 0), 0)
...
export function getPhaseCompletion(
  ...
  for (let w = 1 as 1|2|3|4|5|6|7|8|9; w <= 9; w = ((w+1) as any)) {
...
export function isPhaseEnd(weekNumber: number): boolean { return weekNumber === 9 }
