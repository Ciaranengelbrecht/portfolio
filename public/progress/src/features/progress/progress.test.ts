...
import { getDayCompletion, getWeekCompletion, getPhaseCompletion } from './progress'
...
function makeSession(id: string, week: number, phase=1, reps=8): Session {
  return {
    ...
    phaseNumber: phase,
    phase,
    entries: [{ id: 'e', exerciseId: 'x', sets: [{ setNumber:1, weightKg: 10, reps }] }]
  }
}
...
describe('progress utils', () => {
  ...
  it('weekly target days affects denominator', () => {
    const sessions: Session[] = [makeSession('1-1-0',1), makeSession('1-1-1',1), makeSession('1-1-2',1), makeSession('1-1-3',1)]
    ...
    const w7 = getWeekCompletion(1,1,[...sessions, makeSession('1-1-4',1), makeSession('1-1-5',1), makeSession('1-1-6',1)],{ weeklyTargetDays: 6 })
