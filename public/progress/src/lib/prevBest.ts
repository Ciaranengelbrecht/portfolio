...
export function buildPrevBestMap(sessions: Session[], targetWeek: number, phase: number): PrevBestLookup {
  const map: PrevBestLookup = {}
  const relevant = sessions.filter(s => (s.phaseNumber||s.phase) === phase && s.weekNumber < targetWeek)
