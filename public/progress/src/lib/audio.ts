...
function getCtx(opts: { createIfMissing?: boolean } = {}): AudioContext | null {
  if (ctx) return ctx;
...
export async function unlockAudio() {
  const a = getCtx({ createIfMissing: true });
  if (!a) return false;
...
function playToneAt(time: number, opts: BeepOpts = {}) {
  ...
  if (opts.filter) {
    ...
    if (opts.filter.frequency)
      f.frequency.setValueAtTime(opts.filter.frequency, time);
  ...
  if (opts.shaper) {
    const ws = createWaveshaper(
      a,
      opts.shaper === "hard" ? 28 : 12,
      opts.shaper
    );
    node.connect(ws);
    node = ws;
...
export function playRestBeep(
  pattern: "single" | "double" | "triple" = "double"
) {
  if (!unlocked) return;
  const a = getCtx();
  if (!a) return;
  ...
  playToneAt(t0, { freq: 880, durationMs: 140, volume: 0.1, type: "sine" });
  playToneAt(t0 + 0.2, {
    freq: 1175,
    durationMs: 160,
    volume: 0.1,
    ...
    switch (style) {
      case "gentle": {
        const segs: Seg[] = [
          {
            at: base,
            opts: { freq: 880, durationMs: 150, volume: 0.095, type: "sine" },
          },
          {
            at: base + 0.2,
            opts: { freq: 1175, durationMs: 170, volume: 0.1, type: "sine" },
          },
        ];
        const length = 0.2 + 0.17; // last offset + dur
        return { segs, length };
      }
      case "chime": {
        const segs: Seg[] = [
        ...
        const segs: Seg[] = [
          {
            ...
            opts: {
              ...
              volume: 0.12,
              type: "square",
              filter: { type: "highpass", frequency: 650, Q: 0.8 },
              shaper: "hard",
            },
          },
          {
            ...
            opts: {
              ...
              volume: 0.12,
              type: "square",
              filter: { type: "highpass", frequency: 700, Q: 0.8 },
              shaper: "hard",
            },
          },
        ];
        ...
        const segs: Seg[] = [
          {
            ...
            opts: {
              ...
              volume: 0.14,
              type: "sawtooth",
              filter: { type: "highpass", frequency: 950, Q: 0.9 },
              shaper: "hard",
            },
          },
          {
