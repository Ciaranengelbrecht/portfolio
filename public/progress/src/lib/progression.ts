...
export function suggestNext(ctx: ProgressionContext): NextSetSuggestion | null {
  ...
  const range = parseRange(repRange) ||
    parseRange(exercise.defaults.targetRepRange) || [8, 12];
  const [lo, hi] = range;
  const tops: { weight: number; reps: number }[] = [];
  for (
    let i = recentSessions.length - 1;
    i >= 0 && tops.length < lookbackSessions;
    i--
  ) {
    const s = recentSessions[i];
    const entry = s.entries.find((e) => e.exerciseId === exercise.id);
    if (!entry) continue;
    const logged = entry.sets.filter(
    ...
    const top = logged
      ...
      .sort(
        (a, b) =>
          (b.weightKg || 0) * (b.reps || 0) - (a.weightKg || 0) * (a.reps || 0)
      )[0];
  ...
  if (!adaptive) {
    if (addRepsFirst) {
      ...
      if (last.weight === 0)
        return {
          weightKg: (exercise as any)?.defaults?.startWeight || 0,
          reps: lo,
        };
      if (last.reps < (((lo + hi) / 2) | 0))
        return { weightKg: last.weight, reps: last.reps + 1 };
      return { weightKg: last.weight + incrementKg, reps: lo };
    }
  }
  ...
  const plateau =
    ...
    prev2 &&
    last.reps === prev.reps &&
    prev.reps === prev2.reps &&
    last.reps < hi; // 3 identical outcomes below top
  ...
  if (consecutiveBelowMin) {
    const reduce = Math.max(
      incrementKg,
      Math.round(last.weight * minDeloadPct * 2) / 2
    ); // round to 0.5
  ...
  if (plateau) {
    ...
    if (micro >= 1) {
      return {
        weightKg: last.weight + micro,
        reps: Math.max(lo, Math.min(hi, last.reps)),
      };
    }
...
export function buildSuggestions(
  ...
  const toTime = (iso?: string) => (iso ? Date.parse(iso) || 0 : 0);
  ...
  filtered = filtered.slice().sort((a, b) => {
    const phaseDiff = getPhaseNumber(a) - getPhaseNumber(b);
    if (phaseDiff !== 0) return phaseDiff;
    const weekDiff = getWeekNumber(a) - getWeekNumber(b);
    if (weekDiff !== 0) return weekDiff;
    const dateDiff = toTime(a.dateISO) - toTime(b.dateISO);
    if (dateDiff !== 0) return dateDiff;
    return toTime(a.updatedAt) - toTime(b.updatedAt);
  });

  const current = opts.currentSession || null;
  if (current) {
    const currentPhase = getPhaseNumber(current);
    const currentWeek = getWeekNumber(current);
    filtered = filtered.filter((s) => {
      const phase = getPhaseNumber(s);
      const week = getWeekNumber(s);
      if (phase > currentPhase) return false;
      if (phase === currentPhase && week >= currentWeek) return false;
      return true;
    });
    if (currentWeek === 1 && opts.deloadWeeks?.size) {
      const trimmed = filtered.slice();
      while (trimmed.length) {
        const last = trimmed[trimmed.length - 1];
        const lastPhase = getPhaseNumber(last);
        const lastWeek = getWeekNumber(last);
        if (lastPhase === currentPhase && lastWeek >= currentWeek) {
          trimmed.pop();
          continue;
        }
        if (lastPhase < currentPhase && opts.deloadWeeks.has(lastWeek)) {
          trimmed.pop();
          continue;
        }
        break;
  ...
  for (const ex of exercises) {
    ...
    const sugg = suggestNext({
      recentSessions: recent,
      exercise: ex,
      addRepsFirst: opts.addRepsFirst,
