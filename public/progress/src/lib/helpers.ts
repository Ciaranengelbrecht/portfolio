import { db } from './db'
import { Exercise, Session, SessionEntry, Settings, Measurement } from './types'
import { addDays, subDays, isAfter, parseISO } from 'date-fns'

export async function getLastWorkingSets(exerciseId: string, weekNumber: number, phase?: number) {
  const sessions = await db.getAll<Session>('sessions')
  // try same phase first (if provided), then earlier phases
  const candidates = sessions
    .filter(s => (phase ? s.phase === phase : true) && s.weekNumber < weekNumber && s.entries.some(e => e.exerciseId === exerciseId))
    .sort((a,b) => b.weekNumber - a.weekNumber)
  let prev = candidates[0]
  if (!prev) {
    const older = sessions
      .filter(s => (phase ? (s.phase||1) < phase : true) && s.entries.some(e => e.exerciseId === exerciseId))
      .sort((a,b) => (b.phase||1)-(a.phase||1) || b.weekNumber - a.weekNumber)
    prev = older[0]
  }
...
export async function getDeloadPrescription(exerciseId: string, weekNumber: number) {
  ...
  const loadPct = ex?.defaults.deloadLoadPct ?? (specialW5 ? 0.55 : settings.deloadDefaults.loadPct)
  const setPct = ex?.defaults.deloadSetPct ?? (specialW5 ? 0.5 : settings.deloadDefaults.setPct)
  const avg = sets.length ? sets.reduce((a,b) => a + (b.weightKg||0), 0) / sets.length : 0
...
export async function volumeByMuscleGroup(weekNumber: number) {
  const [sessions, exercises] = await Promise.all([
    db.getAll<Session>('sessions'), db.getAll<Exercise>('exercises')
  ])
  const exMap = new Map(exercises.map(e => [e.id, e]))
  const acc: Record<string, { tonnage: number; sets: number }> = {}
  sessions.filter(s => s.weekNumber === weekNumber).forEach(s => {
...
export async function rollingPRs(exerciseId: string) {
  const sessions = await db.getAll<Session>('sessions')
  const allSets = sessions.flatMap(s => s.entries.filter(e => e.exerciseId===exerciseId).flatMap(e => e.sets))
  let best = 0
...
function sliceByRange<T extends { dateISO?: string; date?: string }>(rows: T[], range: RangeKey) {
  if (range === 'all') return rows
  const weeks = { '4w': 28, '8w': 56, '12w': 84 }[range]
  const cutoff = subDays(new Date(), weeks)
  return rows.filter(r => isAfter(parseISO((r as any).dateISO ?? (r as any).date), cutoff))
}

export async function getExerciseTimeSeries(exerciseId: string, range: RangeKey) {
  const sessions = await db.getAll<Session>('sessions')
  const days = sessions
    .map(s => ({ date: s.dateISO, entry: s.entries.find(e => e.exerciseId === exerciseId) }))
    .filter(x => x.entry)
    .map(x => {
      const sets = (x.entry as SessionEntry).sets
      const topWeight = sets.reduce((m, s) => Math.max(m, s.weightKg), 0)
      const avgWeight = sets.length ? Math.round(sets.reduce((a,b)=>a+b.weightKg,0)/sets.length) : 0
      const repsTotal = sets.reduce((a,b)=>a+b.reps,0)
      const volume = sets.reduce((a,b)=>a+(b.weightKg*b.reps),0)
      return { date: x.date.slice(0,10), topWeight, avgWeight, repsTotal, volume }
    })
    .sort((a,b) => a.date.localeCompare(b.date))
  return sliceByRange(days as any, range)
}

export async function getMeasurementTimeSeries(metric: keyof Measurement, range: RangeKey) {
  const list = (await db.getAll<Measurement>('measurements'))
    .filter(m => (m as any)[metric] != null)
    .map(m => ({ date: m.dateISO.slice(0,10), value: (m as any)[metric] as number }))
    .sort((a,b)=> a.date.localeCompare(b.date))
...
export async function setDashboardPrefs(next: Partial<NonNullable<Settings['dashboardPrefs']>>) {
  const s = await getSettings()
