  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
      ...
      <GlassCard className="relative w-full max-w-md z-10">
        ...
        {msg && (
          <div className="mb-3 text-xs text-gray-300 bg-white/10 rounded-lg px-3 py-2 border border-white/10">{msg}</div>
        )}
        <div className="space-y-2">
          ...
          <div className="flex flex-wrap gap-2">
            <GlossyButton disabled={busy==='signin'} className={`${busy==='signin'?'opacity-70':''}`} onClick={async ()=>{
            ...
            <GlossyButton disabled={busy==='signup'} className={`${busy==='signup'?'opacity-70':''}`} onClick={async ()=>{
              ...
              else { setMsg('Account created'); onSignedIn() }
              setBusy(null)
            }}>Create account</GlossyButton>
            <GlossyButton disabled={busy==='magic'} className={`${busy==='magic'?'opacity-70':''}`} onClick={async ()=>{
              ...
              setBusy(null)
            }}>Send magic link</GlossyButton>
          </div>
          <div className="flex items-center gap-2">
            <input className="bg-slate-800 rounded-xl px-3 py-2 flex-1" placeholder="OTP code" value={otp} onChange={e=>setOtp(e.target.value)} />
            <GlossyButton disabled={busy==='verify'} className={`${busy==='verify'?'opacity-70':''}`} onClick={async ()=>{
          ...
          <div>
            <button className="text-xs underline text-gray-300 hover:text-white" onClick={async ()=>{
              if(!email) return setMsg('Enter your email')
