    ...
    <div className="space-y-6">
      ...
      <div className="bg-card rounded-2xl p-4 shadow-soft space-y-3">
          ...
          {['weightKg','neck','chest','waist','hips','thigh','calf','upperArm','forearm'].map((k) => (
            <label key={k} className="space-y-1">
              ...
              <div className="flex items-center gap-2">
                <button className="bg-slate-700 rounded px-3 py-2" onClick={()=> setM(prev => ({ ...prev, [k]: Math.max(0, Number((prev as any)[k]||0) - 0.5) }))}>-</button>
                <input inputMode="decimal" className="w-full bg-slate-800 rounded-xl px-3 py-3" value={(m as any)[k]||''}
                ...
                <button className="bg-slate-700 rounded px-3 py-2" onClick={()=> setM(prev => ({ ...prev, [k]: Number((prev as any)[k]||0) + 0.5 }))}>+</button>
              </div>
        ...
        <div className="flex items-center gap-3">
          <button className="w-full sm:w-auto bg-brand-600 hover:bg-brand-700 px-3 py-3 rounded-xl" onClick={save}>Save</button>
          ...
          {data.map(row => (
            <div key={row.id} className="bg-slate-800 rounded-xl px-3 py-3">
              <div className="flex items-center justify-between mb-2">
                ...
                <button className="text-xs bg-red-600 rounded px-3 py-2" onClick={()=>remove(row.id)}>Delete</button>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                <div className="bg-slate-900/50 rounded-xl px-2 py-2">
                  ...
                  <div className="flex items-center gap-2">
                    <button className="bg-slate-700 rounded px-3 py-2" onClick={()=> update(row.id, { weightKg: Math.max(0, (row.weightKg||0) - 0.5) })}>-</button>
                    <input className="bg-slate-900 rounded px-3 py-2 w-full text-center" inputMode="decimal" value={row.weightKg||''}
                    ...
                    <button className="bg-slate-700 rounded px-3 py-2" onClick={()=> update(row.id, { weightKg: (row.weightKg||0) + 0.5 })}>+</button>
                  </div>
                </div>
                <div className="bg-slate-900/50 rounded-xl px-2 py-2">
                  <div className="text-[11px] text-gray-400 mb-1">Waist</div>
                  <div className="flex items-center gap-2">
                    <button className="bg-slate-700 rounded px-3 py-2" onClick={()=> update(row.id, { waist: Math.max(0, (row.waist||0) - 0.5) })}>-</button>
                    <input className="bg-slate-900 rounded px-3 py-2 w-full text-center" inputMode="decimal" value={row.waist||''}
                    ...
                    <button className="bg-slate-700 rounded px-3 py-2" onClick={()=> update(row.id, { waist: (row.waist||0) + 0.5 })}>+</button>
                  </div>
                </div>
                <div className="bg-slate-900/50 rounded-xl px-2 py-2">
                  <div className="text-[11px] text-gray-400 mb-1">Upper Arm</div>
                  <div className="flex items-center gap-2">
                    <button className="bg-slate-700 rounded px-3 py-2" onClick={()=> update(row.id, { upperArm: Math.max(0, (row.upperArm||0) - 0.5) })}>-</button>
                    <input className="bg-slate-900 rounded px-3 py-2 w-full text-center" inputMode="decimal" value={row.upperArm||''}
                    ...
                    <button className="bg-slate-700 rounded px-3 py-2" onClick={()=> update(row.id, { upperArm: (row.upperArm||0) + 0.5 })}>+</button>
                  </div>
                </div>
              </div>
            </div>
