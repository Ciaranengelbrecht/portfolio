...
export default function SettingsPage() {
  ...
  const exportData = async () => {
    await triggerExportDownload({ includeRawJson: true, prettyJson: true, excludeEmptySessions: true });
  };
  ...
  const importData = async (file: File) => {
    ...
    if(file.name.endsWith('.json')){
      ...
      if(/manifest\.json/.test(text.substring(0,200))) {
        alert('Please extract zip and import raw.json (zip parsing not implemented inline).');
        return;
      }
      result = await importFromRawJson(text);
    }
    alert(`Imported: ${Object.entries(result.inserted).map(([k,v])=> `${k}=${v}`).join(' ')}`);
  };
